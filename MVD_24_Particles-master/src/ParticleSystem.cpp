...
void ParticleSystem::createParticle(ParticleEmitter& particleEm) {
	...
	std::vector<GLfloat> vertices(particleEm.num_particles * 3, 0);
	std::vector<GLfloat> velocities(particleEm.num_particles * 3, 0);
	std::vector<GLfloat> ages(particleEm.num_particles, 0);
	std::vector<GLfloat> lives(particleEm.num_particles, 0);

	for (int i = 0; i < particleEm.num_particles; i++) {
...
void ParticleSystem::update() {
	...
	Transform& trans = ECS.getComponentFromEntity<Transform>("Snow");
	lm::mat4 pos = trans;
