...
void DebugSystem::imGuiRenderTransformNode(TransformNode& trans) {
	...
	if (ImGui::TreeNode(ent.name.c_str())) {
		...
		ImGui::Text("Position");
		ImGui::DragFloat3("", pos_array);
		transform.position(pos_array[0], pos_array[1], pos_array[2]);
		ImGui::NewLine();
		ImGui::Text("Rotation");

		if (ImGui::SliderFloat(" :rX", &angleX, 0.0f, 360.f, "%0.3f", 1.0)) {
			float degreesX = 0;
			if (angleX >= prevXdegree) {
				degreesX = angleX - prevXdegree;
				degreesX *= 0.01745;
				prevXdegree = angleX;
				transform.rotateLocal(degreesX, lm::vec3(1, 0, 0));
			}
			else {
				degreesX = prevXdegree - angleX;
				degreesX *= 0.01745;
				prevXdegree = angleX;
				transform.rotateLocal(degreesX, lm::vec3(-1, 0, 0));
			}
		}
		if (ImGui::SliderFloat(" :rY", &angleY, 0.0f, 360.f, "%0.3f", 1.0)) {
			float degreesY = 0;
			if (angleY >= prevYdegree) {
				degreesY = angleY - prevYdegree;
				degreesY *= 0.01745;
				prevYdegree = angleY;
				transform.rotateLocal(degreesY, lm::vec3(0, 1, 0));
			}
			else {
				degreesY = prevYdegree - angleY;
				degreesY *= 0.01745;
				prevYdegree = angleY;
				transform.rotateLocal(degreesY, lm::vec3(0, -1, 0));
			}
		...
		if (ImGui::SliderFloat(" :rZ", &angleZ, 0.0f, 360.f, "%0.3f", 1.0)) {
			float degreesZ = 0;
			if (angleZ >= prevZdegree) {
				degreesZ = angleZ - prevZdegree;
				degreesZ *= 0.01745;
				prevZdegree = angleZ;
				transform.rotateLocal(degreesZ, lm::vec3(0, 0, 1));
			}
			else {
				degreesZ = prevZdegree - angleZ;
				degreesZ *= 0.01745;
				prevZdegree = angleZ;
				transform.rotateLocal(degreesZ, lm::vec3(0, 0, -1));
			}
		...
		if (scaleX < 1.0 || scaleY < 1.0 || scaleZ < 1.0 || prevXscale < 1.0 || prevYscale < 1.0 || prevZscale < 1.0) {
			scaleX = scaleY = scaleZ = prevXscale = prevYscale = prevZscale = 1.0;
		}

		ImGui::NewLine();
		ImGui::Text("Scale");
		if (ImGui::SliderFloat(" :sX", &scaleX, 1.0f, 10.0f, "%0.3f", 1.0f)) {
			if (scaleX >= 1) {
				transform.scaleLocal(1 / prevXscale, 1, 1);
				transform.scaleLocal(scaleX, 1, 1);
				prevXscale = scaleX;
			}
		}
		if (ImGui::SliderFloat(" :sY", &scaleY, 1.0f, 10.0f, "%0.3f", 1.0f)) {
			if (scaleY >= 1) {
				transform.scaleLocal(1, 1 / prevYscale, 1);
				transform.scaleLocal(1, scaleY, 1);
				prevYscale = scaleY;
			}
		}
		if (ImGui::SliderFloat(" :sZ", &scaleZ, 1.0f, 10.0f, "%0.3f", 1.0f)) {
			if (scaleZ >= 1) {
				transform.scaleLocal(1, 1, 1 / prevZscale);
				transform.scaleLocal(1, 1, scaleZ);
				prevZscale = scaleZ;
			}
		}
	
		if (ECS.hasComponent<Light>(transform.owner)) {
			Light& l = ECS.getComponentFromEntity<Light>(transform.owner);
			ImGui::NewLine();
			if (l.type == 0) {
				float dir[3] = { l.direction.x, l.direction.y, l.direction.z };
				ImGui::DragFloat3("Direction", dir, 1.0, -1.0, 1.0);
				l.direction = lm::vec3(dir[0], dir[1], dir[2]);

			}
			if (l.type == 1) {
				linear_att = l.linear_att;
				quadratic_att = l.quadratic_att;
				ImGui::SliderFloat("Linear Attenuation", &linear_att, 0.0, 1.0, "%.4f");
				ImGui::SliderFloat("Quadratic Attenuation", &quadratic_att, 0.0, 1.0, "%.4f");
				l.linear_att = linear_att;
				l.quadratic_att = quadratic_att;
			}
			if (l.type == 2) {
				linear_att = l.linear_att;
				quadratic_att = l.quadratic_att;
				spot_inner = l.spot_inner;
				spot_outer = l.spot_outer;
				ImGui::SliderFloat("Linear Attenuation", &linear_att, 0.0, 1.0, "%.4f");
				ImGui::SliderFloat("Quadratic Attenuation", &quadratic_att, 0.0, 1.0, "%.4f");
				ImGui::SliderFloat("Inner Spot", &spot_inner, 1.0, 100, "%.3f");
				ImGui::SliderFloat("Outer Spot", &spot_outer, 1.0, 100, "%.3f");
				l.linear_att = linear_att;
				l.quadratic_att = quadratic_att;
				if (spot_inner <= spot_outer) {
					spot_inner = spot_outer + 1;
				}
				l.spot_inner = spot_inner;
				l.spot_outer = spot_outer;
			}
		}
...
//        }
		...
		if (ImGui::TreeNode("Sky Light")) {
			...
			if (ImGui::RadioButton("Day light",(bool)true)) {
				graphics_system_->opacity = 1.0;
			}
			ImGui::SameLine();
			if (ImGui::RadioButton("Night light", (bool)true)) {
				graphics_system_->opacity = 0.3;
			}
			ImGui::TreePop();
		}

		if (ImGui::TreeNode("Weather updates")) {

			int index = ECS.getEntity("Terrain");
			Mesh& terrain_mesh = ECS.createComponentForEntity<Mesh>(index);
			Material& mat = graphics_system_->getMaterial(terrain_mesh.material);
			Transform& trans = ECS.getComponentFromEntity<Transform>(index);
			TransformNode tn;
			tn.trans_id = ECS.getComponentID<Transform>(index);
			tn.entity_owner = trans.owner;
			tn.ent_name = ECS.entities[tn.entity_owner].name;
			imGuiRenderTransformNode(tn);
			ImGui::Text("Snow height: ");
			ImGui::SameLine();
			ImGui::SliderFloat(" ", &mat.height , 0.0, 50.0, 0);
			ImGui::Text("Snowing?");
			if (ImGui::RadioButton("Yes", (bool)true)) {
				graphics_system_->particlesOn = true;
			}
			ImGui::SameLine();
			if (ImGui::RadioButton("No", (bool)true)) {
				graphics_system_->particlesOn = false;
			}
			Transform& trans1 = ECS.getComponentFromEntity<Transform>(ECS.getEntity("Snow"));
			TransformNode tn1;
			tn1.trans_id = ECS.getComponentID<Transform>(ECS.getEntity("Snow"));
			tn1.entity_owner = trans1.owner;
			tn1.ent_name = ECS.entities[tn1.entity_owner].name;
			imGuiRenderTransformNode(tn1);
			ImGui::TreePop();

		}

		if (ImGui::TreeNode("Blend Shape")) {

			if (ImGui::TreeNode("Face")) {

				Transform& trans = ECS.getComponentFromEntity<Transform>(ECS.getEntity("toon"));
				TransformNode tn;
					tn.trans_id = ECS.getComponentID<Transform>(ECS.getEntity("toon"));
					tn.entity_owner = trans.owner;
					tn.ent_name = ECS.entities[tn.entity_owner].name;
				imGuiRenderTransformNode(tn);
		...
		if (ImGui::TreeNode("Animations")) {
			...
			for (auto& anim : anims) {
				string animName = ECS.getEntityName(anim.owner);
				if (ImGui::TreeNode(animName.c_str())) {
					if (ImGui::RadioButton("Start", (bool)true)) {
						anim.curr_frame = 0;
						anim.active = true;
					}
					ImGui::SameLine();
					if (ImGui::RadioButton("Continue", (bool)true)) {
						anim.active = true;
					}
					ImGui::SameLine();
					if (ImGui::RadioButton("Stop", (bool)true)) {
						anim.active = false;
					}
					ImGui::SliderFloat("Animation Speed", &anim.ms_frame, 20, 60, 0);
				}
				ImGui::TreePop();
			}
			...
			auto& skinnedmeshes = ECS.getAllComponents<SkinnedMesh>();
			for (auto& sm : skinnedmeshes) {
				string animName = ECS.getEntityName(sm.owner);
				if (ImGui::TreeNode(animName.c_str())) {
					if (ImGui::RadioButton("Start", (bool)true)) {
						sm.active = -1;
					}
					...
					ImGui::SliderFloat("Animation Speed", &sm.ms_frame, 20, 60, 0);
				}
				ImGui::TreePop();
			}
		}

		if (ImGui::TreeNode("Material Sets")) {
			Transform& trans = ECS.getComponentFromEntity<Transform>(ECS.getEntity("Suit"));
			TransformNode tn;
			tn.trans_id = ECS.getComponentID<Transform>(ECS.getEntity("Suit"));
			tn.entity_owner = trans.owner;
			tn.ent_name = ECS.entities[tn.entity_owner].name;
			imGuiRenderTransformNode(tn);
			ImGui::TreePop();
		...
		if (ImGui::TreeNode("Follow the path (IA)")) {
			ImGui::Text("INSTRUCTIONS:");
			const ImVec4 red(1.0, 0.0, 0.0, 1.0);
			ImGui::TextColored(red,"To get a red tile (End point) press: 1");
			const ImVec4 green(0.0, 1.0, 0.0, 1.0);
			ImGui::TextColored(green,"To get a green tile (Start point) press: 2");
			const ImVec4 white(1.0, 1.0, 1.0, 1.0);
			ImGui::TextColored(white, "To get a white tile (Floor) press: 3");
			const ImVec4 purple(0.5, 0.0, 0.5, 1.0);
			ImGui::TextColored(purple,"To get a pruple tile (Non-walkable floor) press: 4");
			ImGui::Text("When the tiles are black, you can edit them with the previous mentioned buttons");
			ImGui::Text("Press R to restart the path");
			ImGui::TreePop();
		}

		if (ImGui::TreeNode("Render Mode")) {
			if (ImGui::RadioButton("Deferred Rendering", (bool)true)) {
				graphics_system_->renderMode = true;
			}
			ImGui::SameLine();
			if (ImGui::RadioButton("Volumetric lighting", (bool)true)) {
				graphics_system_->renderMode = false;
			}
			ImGui::TreePop();
		}

		if (ImGui::TreeNode("Lights")) {
			if (ImGui::TreeNode("Lights in scene")) {
				auto& lights = ECS.getAllComponents<Light>();
				for (auto& l : lights) {
					Transform& trans = ECS.getComponentFromEntity<Transform>(l.owner);
					TransformNode tn;
					tn.trans_id = ECS.getComponentID<Transform>(l.owner);
					tn.entity_owner = trans.owner;
					tn.ent_name = ECS.entities[tn.entity_owner].name;
					imGuiRenderTransformNode(tn);
				}
			...
			if (ImGui::TreeNode("Create new light")) {
				...
				case 1:
					ImGui::DragFloat3("Position", pos_array, 1.0, -1000, 1000);
					ImGui::SliderFloat3("Color", color, 0, 1, "%.3f");
					ImGui::SliderFloat("Linear Attenuation", &linear_att, 0.0, 1.0, "%.3f");
					ImGui::SliderFloat("Quadratic Attenuation", &quadratic_att, 0.0, 1.0, "%.3f");
					type = LightTypePoint;
					break;
				case 2:
					ImGui::DragFloat3("Position", pos_array, 1.0, -1000, 1000);
				...
				if (ImGui::Button("Create")) {
					auto& lightsList = ECS.getAllComponents<Light>();

					int new_light = ECS.createEntity("light_" + to_string(lightsList.size()));
					ECS.getComponentFromEntity<Transform>(new_light).translate(pos_array[0], pos_array[1], pos_array[2]);
					Light& light = ECS.createComponentForEntity<Light>(new_light);
					light.type = type; //change for direction or spot
					light.position = lm::vec3(0, 0, 0);
					light.color = lm::vec3(color[0], color[1], color[2]);
					if (pos == 0) {
						light.direction = lm::vec3(direction[0], direction[1], direction[2]);
