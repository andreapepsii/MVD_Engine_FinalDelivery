...
void main(){
	...
	vec2 s_uv_1 = v_uv * u_uv_scale * 0.1;
	vec3 grass_low_res = texture(u_diffuse_map, s_uv_1).xyz;
	 
	vec2 s_uv_2 = v_uv * u_uv_scale * 0.5;
	vec3 grass_med_res = texture(u_diffuse_map, s_uv_2).xyz;


	vec3 cliffs = texture(u_diffuse_map_2, s_uv_2).xyz;
	//mat_diffuse = cliffs;
	
	mat_diffuse *= mix(mix(grass_full_res, grass_low_res, 0.5), grass_med_res, 0.5);

	mat_diffuse = mix(cliffs,mat_diffuse, clamp( N.y * N.y, 0, 1));

	float snow_mix = v_vertex_world_pos.y / u_max_height;

	float random_snow = texture(u_noise_map, s_uv_1 * 2).x;

	snow_mix = clamp(snow_mix * 3 - 1.5 + random_snow, 0.0, 1.0);
